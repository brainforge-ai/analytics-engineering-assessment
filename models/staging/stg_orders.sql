...
from (
    select 
        ...
        ,max(try_strptime(trim(updated_at::varchar), '%Y-%m-%d')::date) over (partition by order_id order by updated_at desc rows between unbounded preceding and unbounded following) as max_updated_at
